...
{
    if (level == 1)
        el->t = vec_dot(vec_minus(sq->pos, intx->ray.orig), sq->dir) / vec_dot(intx->ray.dir, sq->dir);
    if (level == 2)
    {
        ...
        el->v3 = vec_add(sq->pos, vec_scale(vec_cross(sq->dir, sq->dir), sq->side_size));
        el->v4 = vec_minus(sq->pos, vec_scale(vec_cross(sq->dir, sq->dir), sq->side_size));
    }
}
...
void init_cylinder_intx(t_cy_intx *el, t_intx *intx, t_cy *cy, int level)
{
    if (level == 1)
    {
        ...
        el->a = vec_dot(intx->ray.dir, intx->ray.dir) - vec_dot(intx->ray.dir, cy->dir) * vec_dot(intx->ray.dir, cy->dir);
        el->b = 2 * (vec_dot(el->oc, intx->ray.dir) - vec_dot(el->oc, cy->dir) * vec_dot(intx->ray.dir, cy->dir));
        el->c = vec_dot(el->oc, el->oc) - vec_dot(el->oc, cy->dir) * vec_dot(el->oc, cy->dir) - cy->radius * cy->radius;
        el->discriminant = el->b * el->b - 4 * el->a * el->c;
    }
    if (level == 2)
    {
...
void init_triangle_intx(t_tr_intx *el, t_intx *intx, t_tr *tr, int level)
{
    ...
    if (level == 2)
    {
        el->invDet = 1.0 / el->det;
        el->tvec = vec_minus(intx->ray.orig, tr->pos1);
        el->u = vec_dot(el->tvec, el->pvec) * el->invDet;
    }
    if (level == 3)
    {
        el->qvec = vec_cross(el->tvec, el->edge1);
        el->v = vec_dot(intx->ray.dir, el->qvec) * el->invDet;
    }
    if (level == 4)
        el->t = vec_dot(el->edge2, el->qvec) * el->invDet;
}