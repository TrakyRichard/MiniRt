...
// Function to trace a ray through the scene and calculate the color at the intersection point
void ray_trace(t_sc scene)
{
    ...
    for (int y = 0; y < scene.r.h; y++)
    {
        for (int x = 0; x < scene.r.w; x++)
        {
            ...
            /*       double u = (2.0 * (x + 0.5) / scene.r.w - 1) * scene.c.w / 2;
                  double v = (1 - 2.0 * (y + 0.5) / scene.r.h) * scene.c.h / 2; */

            // Create a ray with the calculated direction and the camera position
            t_ray ray;
